...
(async() => {
    ...
    // Функция для получения подарков канала
    async function getChannelStarGifts(channelUsername) {
        try {
            console.log(`\n📺 Получаю список подарков канала @${channelUsername}...\n`);
            
            // Получаем InputPeer для канала
            const channelPeer = await client.getInputEntity(channelUsername);
            ...
            console.log(`📊 Всего подарков у канала @${channelUsername}: ${result.count}`);
            console.log('═'.repeat(70));

            if (result.gifts.length === 0) {
                console.log('\n😔 У канала нет подарков\n');
                return result;
            }
            ...
            result.gifts.forEach((savedGift, index) => {
                ...
                if (gift.className === 'StarGiftUnique') {
                    ...
                    if (gift.ownerAddress) {
                        console.log(`Адрес владельца: ${gift.ownerAddress}`);
                    }
                    ...
                    if (gift.resellAmount) {
                        ...
                        gift.resellAmount.forEach(amount => {
                            if (amount.currency) {
                                console.log(`  • ${amount.amount} ${amount.currency.toUpperCase()}`);
                            } else {
                                console.log(`  • ${amount.amount} stars`);
                            }
                        });
                    }
                    ...
                    if (gift.valueAmount && gift.valueCurrency) {
                        console.log(`💵 Стоимость: ${gift.valueAmount} ${gift.valueCurrency}`);
                    }
                ...
                // Статусы
                const statuses = [];
                if (savedGift.pinnedToTop) statuses.push('📌 Закреплен');
                if (savedGift.unsaved) statuses.push('❌ Не сохранен');
                if (savedGift.refunded) statuses.push('💸 Возвращен');
                if (savedGift.canUpgrade) statuses.push('⬆️ Можно улучшить');
            ...
            console.error(`❌ Ошибка при получении подарков канала @${channelUsername}:`, error);
            throw error;
        }
    }
    ...
    // Получаем подарки канала @fiscaldev
    await getChannelStarGifts('fiscaldev');
    ...
    */
    ...
    client.addEventHandler(async(event) => {
        try {
            ...
            // Команда для получения подарков канала
            if (text.toLowerCase().includes('/channel') || text.toLowerCase().includes('/канал')) {
                console.log('📝 Получена команда для вывода подарков канала');
                await getChannelStarGifts('fiscaldev');
                return;
            }

            // Команда для получения подарков любого канала
            // Формат: /getchannel username или /канал username
            const channelMatch = text.match(/\/(getchannel|получить)\s+@?(\w+)/i);
            if (channelMatch) {
                const username = channelMatch[2];
                console.log(`📝 Получена команда для вывода подарков канала @${username}`);
